  ...
  main.innerHTML = `
    ...
    <div class="btn-grid">
      ${subjects[grade]
        .map(
          (sub) =>
            `<div class="btn" onclick="selectSubject(${grade}, '${sub}')">${sub}</div>`
        )
        .join('')}
    </div>
  ...
  main.innerHTML = `
    ...
    <div class="btn-grid">
      ${[2025, 2024, 2023]
        .map(
          (y) =>
            `<div class="btn" onclick="selectYear(${grade}, '${subject}', ${y})">${y}년</div>`
        )
        .join('')}
    </div>
  ...
  main.innerHTML = `
    ...
    <div class="btn-grid">
      ${['1학기 중간고사', '1학기 기말고사', '2학기 중간고사', '2학기 기말고사']
        .map(
          (t) =>
            `<div class="btn" onclick="showAnswers(${grade}, '${subject}', ${year}, '${t}')">${t}</div>`
        )
        .join('')}
    </div>
