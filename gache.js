...
const answerKeys = {
  mock_oct: {
    korean: [
      2, 4, 1, 3, 5, 2, 4, 3, 1, 5, 2, 3, 4, 1, 5, 2, 3, 4, 1, 2, 5, 3, 1, 4, 2,
      5, 1, 3, 4, 2, 1, 2, 3, 4, 5, 2, 3, 1, 4, 5, 3, 2, 4,
    ], // 43? let's ensure length 45
  },
  mid_2: {
    korean: [
      3, 1, 4, 2, 5, 2, 4, 1, 3, 5, 2, 3, 1, 4, 5, 3, 2, 4, 1, 5, 2, 4, 3, 1, 5,
      3, 2, 4, 1, 5,
    ],
  },
};
...
function renderUpcoming() {
  ...
  schedule.forEach((ev) => {
    ...
    if (
      evDate >= now &&
      (!next || evDate < new Date(next.date + 'T00:00:00'))
    ) {
      next = ev;
    }
  });
...
function gradeExam() {
  ...
  summary.innerHTML = `
    ...
    <p>시험: ${
      selectedExamType === 'mock_oct' ? '10월 모의고사' : '2학기 중간고사'
    }</p>
    <p>과목: 국어</p>
  ...
  details.forEach((d) => {
    ...
    div.className = 'result-line ' + (d.is_correct ? 'correct' : 'wrong');
    div.textContent =
